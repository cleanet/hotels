...
* **com.myenterprise.rest.annotation.sanitizehtml**
  * **SanitizeHtml**: Annotation definition
  * **SanitizePolicy**: A policy component for sanitizing HTML content generated by CKEditor.
  * **SanitizeHtmlRequestBody**: Deserializer that sanitizes HTML content in request bodies after they are read and before they are passed to controller methods.
  * **SanitizerHTMLResponse**: Serializer that sanitizes HTML content in response bodies before they are written to the HTTP response.
  * * **SanitizerHtml**: provides utility methods for sanitizing HTML content.
...
#### Tests
You can test the sanitization in `/src/test/java/com/myenterprise/rest/annotation/sanitizehtml`:
* JsonBenchmarkTest: Test the performance of serialization/deserialization with sanitization with responseList.json
* JsonDynamicBenchmarkTest: Test the performance of serialization/deserialization with sanitization with JSON generated on the fly
* JsonGenerator: Responsible for generate the Json with the elements passed by arguments.
* SanitizeHtmlDynamicTest: Test the proper functioning of deserializer with sanitization. With Json dynamic
