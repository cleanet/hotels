...
CONFIG OPTIONS
	...
	optionalAcceptNullable
	    Use `ofNullable` instead of just `of` to accept null values when using Optional. (Default: true)
	...
	useFeignClientContextId
	    Whether to generate Feign client with contextId parameter. (Default: true)
	...
	useSealed
	    Whether to generate sealed model interfaces and classes (Default: false)
	...
	useSpringBuiltInValidation
	    Disable `@Validated` at the class level when using built-in validation. (Default: false)
